...
@Component({
  ...
  styleUrl: './defaulters.component.css'
})
export class DefaultersComponent implements AfterViewInit{
  ...
  notReturnedBkDSource = new MatTableDataSource(ELEMENT_DATA);

  @ViewChild(MatTable) table?: MatTable<issueBookTable>;
  @ViewChild(MatPaginator) paginator: MatPaginator;
  @ViewChild(MatSort) sort: MatSort;

  constructor(private studentService: StudentServiceService, private issueBookService: IssueBookService, private returnBookService: ReturnBookServiceService) {
    this.getAllIssueBooks();
  }

  ngAfterViewInit() {
    this.notReturnedBkDSource.paginator = this.paginator;
    this.notReturnedBkDSource.sort = this.sort;
  }

  applyFilter(event: Event) {
    const filterValue = (event.target as HTMLInputElement).value;
    this.notReturnedBkDSource.filter = filterValue.trim().toLowerCase();

    if (this.notReturnedBkDSource.paginator) {
      this.notReturnedBkDSource.paginator.firstPage();
    }
  }
  ...
  getAllIssueBooks() {
    ...
    this.issueBookService.getIssueBooks().subscribe((response) => {
      ...
      this.notReturnedBkDSource.data = [];
      ...
      response.forEach((issueBook: issue_Book) => {
        ...
        this.notReturnedBkDSource.data.push(obj);
        console.log(this.notReturnedBkDSource.data);
        }
      });
  ...
  return_book(issueBookTid: number) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.notReturnedBkDSource.data.forEach(element => {
